...
class Company extends Model {
    ...
    public static function sortByOrder($companies, $order) {
        if ($order == 'company_rating_high') {
			$companies->sortByDesc(function($company) {
				return $company->recommend_percent();
			});
		}
		elseif ($order == 'company_rating_low') {
			$companies->sortBy(function($company) {
				return $company->recommend_percent();
			});
		}
        elseif ($order == 'num_reviews_high') {
			$companies->sortByDesc(function($company) {
				return count($company->approvedReviews()->get());
			});
		}
